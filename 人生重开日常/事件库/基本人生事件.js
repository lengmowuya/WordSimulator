class 基本人生事件库 extends 事件库模板 {
    constructor(){
        super();
        this.执行列表 = ['出生','成长','衰老','死亡','成人','学校','社畜','毕业','生娃','添加随机选择词条']
        this.name = '基本人生事件库';
    }
    出生(){
    ...
    学校(){
        if(属性.age < 6) return;
        抉择中心.创建抉择窗口('父母要求你去上学',[{text:'乖乖顺从'},{text:'愤然拒绝!'}]);
        订阅事件('愤然拒绝!',()=>{
            添加词条("[父母要求你去上学] 你的鬼哭狼嚎和挣扎只换来一顿混合双打,最后你还是屈服了");
            return true;
        })
        订阅事件('乖乖顺从',()=>{
            设置属性({isStudent:true});
            if(属性.家境<= 3){
                添加词条("[父母要求你去上学] 你上学了,是村办学校");
            }else if(属性.家境 > 7){
                添加词条("[父母要求你去上学] 你上学了是母亲托关系加重金办理的市重点学校");
            }else{
                添加词条("[父母要求你去上学] 你上学了,是镇上的公立学校");
            }
            return true;
        })
        return true;
